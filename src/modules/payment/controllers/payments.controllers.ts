...
export class PaymentsControllers {
  ...
  handleWebhook = async (req: Request, res: Response) => {
    try {
      ...
      // Require raw buffer — if body is not a Buffer the signature check will always fail
      if (!Buffer.isBuffer(req.body)) {
        ResponseUtil.badRequest(res, 'Invalid webhook body', 'Webhook route must use express.raw() middleware, not express.json()');
        return;
      }

      const rawBody = req.body.toString('utf8');

      const isValid = this.paymentServices.verifyWebhookSignature(rawBody, signature);
      if (!isValid) {
      ...
      await this.paymentServices.processWebhook(event);
